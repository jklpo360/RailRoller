...
func assign_home_city(player):
	...
	var roll1 = []
	roll1.append(blackdice1)
	roll1.append((dice11 + dice21))
	var roll2 = []
...
func choose_destination(player): 
	...
	if random_region == player_regions[player]:
		not_waiting = false
		GlobalSignals.request_region.emit(player)
		await response
		not_waiting = true
...
func print_reward(origin, destination, origin_region, destination_region):
	...
	var destination_number = alphabetical_cities.get(destination_region).find(destination)
...
func load_game() -> void:
	...
	for i in range(NUM_PLAYERS):
		...
		InputMap.action_add_event(action, primary_events[i])
